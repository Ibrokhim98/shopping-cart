...
async function addCart(data) {
    const response = await fetch(`${URL}/shopping-carts`, {
        method: 'POST',
        headers: {
...
async function updateCart(id, data) {
    const response = await fetch(`${URL}/shopping-carts/${id}`, {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/json',
            Authorization: accessToken,
...
async function checkoutOrder() {
    const response = await fetch(`${URL}/shopping-carts/checkout`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            Authorization: accessToken,
...
async function setProductTable() {
    ...
    if (productsResponse && productsResponse?.length > 0) {
        ...
        productsResponse.forEach((product) => {
            ...
            btnCol.innerHTML = `<button data-id="${product.id}" onclick="cartButton(this)" class="btn btn-success" title="Add to cart"><i class="bi bi-bag-plus"></i> Add</button>`;
